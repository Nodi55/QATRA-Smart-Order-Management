...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 3. إيقاف/تفعيل الموظف - عند الإيقاف تُسحب مهامه النشطة تلقائياً وتصبح "غير مسندة"
    if (isset($_POST['toggle_status'])) {
        ...
        if ($empInfo['is_active'] == 1) {
            // الموظف نشط الآن وسيتم إيقافه: نسحب منه أي مهام فحص/تركيب لم تُنجز بعد
            $pulledCount = 0;

            $fiStmt = $pdo->prepare("SELECT insp_id FROM field_inspection WHERE emp_id = ? AND inspection_result IS NULL");
            $fiStmt->execute([$eId]);
            $fiIds = $fiStmt->fetchAll(PDO::FETCH_COLUMN);
            if (!empty($fiIds)) {
                $pdo->prepare("DELETE FROM field_inspection WHERE insp_id IN (" . implode(',', array_map('intval', $fiIds)) . ")")->execute();
                $pulledCount += count($fiIds);
            }

            try {
                $itStmt = $pdo->prepare("SELECT app_id FROM installation_task WHERE emp_id = ? AND initial_reading IS NULL");
                $itStmt->execute([$eId]);
                $itAppIds = $itStmt->fetchAll(PDO::FETCH_COLUMN);
                if (!empty($itAppIds)) {
                    $pdo->prepare("DELETE FROM installation_task WHERE emp_id = ? AND initial_reading IS NULL")->execute([$eId]);
                    $pulledCount += count($itAppIds);
                }
            } catch (Exception $e) {}

            $pdo->prepare("UPDATE company_employee SET is_active = 0, active_tasks_count = 0 WHERE emp_id = ?")->execute([$eId]);

            if ($pulledCount > 0) {
                $msg = "تم إيقاف الموظف بنجاح، وتم سحب (" . $pulledCount . ") مهمة نشطة منه وإعادتها لقائمة \"مهام تحتاج إسناد\" ليتم توزيعها على فنيين آخرين.";
                $msgType = "warning";
            } else {
                $msg = "تم إيقاف الموظف بنجاح، ولم يكن لديه أي مهام نشطة."; $msgType = "success";
            }
        } else {
            // الموظف موقوف الآن وسيتم تفعيله
            $pdo->prepare("UPDATE company_employee SET is_active = 1 WHERE emp_id = ?")->execute([$eId]);
            $msg = "تم تفعيل الموظف بنجاح."; $msgType = "success";
        }
    }
