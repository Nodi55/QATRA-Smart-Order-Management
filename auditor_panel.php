...
if (isset($_GET['read_notif'])) {
    ...
    header("Location: auditor_panel.php");
    exit;
}
    ...
    if (isset($_POST['approve_app'])) {
        ...
        try {
            ...
            if ($assigned) {
                ...
                // إرسال إشعار فوري للفني إذا تم إسناد المهمة له في مدينة أخرى
                $isDiff = ($assigned['cty_id'] != $cityId);
                if ($isDiff) {
                    ...
                    $empNotifMsg = "لديك مهمة فحص ميداني جديدة رقم #" . str_pad($appId, 5, '0', STR_PAD_LEFT) . " خارج نطاق مدينتك المعتادة في (مدينة " . $cleanCty . ")!";
                    $pdo->prepare("INSERT INTO employee_notification (emp_id, message_content, notif_type) VALUES (?, ?, 'new_task_different_city')")
                        ->execute([$assigned['emp_id'], $empNotifMsg]);
                }
...
<!-- نافذة التدقيق الشاملة والمنبثقة مع عارض الصك المباشر لمنع Not Found -->
...
<div style="height: 220px; border-radius: 12px; overflow:hidden; border: 1px solid #e2e8f0;">
...
<div class="mb-3">
    <div id="deed_preview_box" style="height: 250px; border: 1px solid #cbd5e1; border-radius: 12px; overflow: hidden; background: #fafbfc;">
        <!-- يتم توليد المعاينة بداخل الجافاسكريبت تلقائياً -->
    </div>
</div>

<h6 class="fw-bold mb-1 text-danger"><i class="fa-solid fa-triangle-exclamation me-1"></i> في حال رفض الطلب:</h6>
<textarea id="rejection_input" class="form-control mb-3" rows="2" placeholder="اكتب هنا سبب الرفض بالتفصيل..." required></textarea>
</div>
