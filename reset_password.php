...
if (!isset($_SESSION['temp_reset_cust_id'])) {
...
$errorMsg = "";
$custId = $_SESSION['temp_reset_cust_id'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen($newPassword) < 6) {
        ...
        $validOtp = null;
        try {
            $stmt = $pdo->prepare("SELECT otp_id FROM otp_code WHERE cust_id = ? AND code = ? AND is_used = 0 AND purpose = 'reset' AND expiry_time > NOW() ORDER BY otp_id DESC LIMIT 1");
        ...
        if ($validOtp) {
            ...
            try {
                $pdo->prepare("UPDATE customer SET password_hash = ? WHERE cust_id = ?")->execute([$hashedPassword, $custId]);
                $pdo->prepare("UPDATE otp_code SET is_used = 1 WHERE otp_id = ?")->execute([$validOtp['otp_id']]);

                unset($_SESSION['temp_reset_cust_id'], $_SESSION['temp_reset_national_id'], $_SESSION['temp_reset_otp']);
...
<head>
    ...
    <title>إعادة تعيين كلمة المرور | نظام قطرة</title>
    ...
    <style>
        ...
        .card-box { background: rgba(255, 255, 255, 0.97); backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 24px; padding: 45px 50px; width: 100%; max-width: 460px; z-index: 10; box-shadow: 0 25px 60px rgba(0, 0, 0, 0.4); position: relative; }
        .card-box::before { content: ''; position: absolute; top: 0; left: 10%; width: 80%; height: 4px; background: linear-gradient(90deg, transparent, var(--qatra-light), transparent); border-radius: 0 0 10px 10px; }
...
<div class="card-box">
    ...
    <div class="footer-links">
        <a href="forgot_password.php"><i class="fa-solid fa-rotate-left me-1"></i> إعادة إرسال الرمز</a>
    </div>
</div>
