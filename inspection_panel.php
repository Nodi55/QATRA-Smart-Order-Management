    ...
    if (isset($_POST['approve_app'])) {
        ...
        try {
            ...
            if ($assigned) {
                ...
                // إذا تم إسناد المهمة لفني في مدينة أخرى، يتم إرسال إشعار فوري للفني
                if ($assigned['cty_id'] != $cityId) {
                    $cityStmt = $pdo->prepare("SELECT cty_name FROM city WHERE cty_id = ?");
                    $cityStmt->execute([$cityId]);
                    $cityName = $cityStmt->fetchColumn() ?? 'مدينة أخرى';
                    $cleanCityName = str_replace('مدينة ', '', $cityName);
                    
                    $empNotifMsg = "لديك مهمة فحص ميداني جديدة رقم #" . str_pad($appId, 5, '0', STR_PAD_LEFT) . " خارج نطاق مدينتك المعتادة في (مدينة " . $cleanCityName . ")!";
