...
// التحقق من تسجيل الدخول للمستفيد
...
// =========================================================
// 1. معالجة إرسال طلب جديد عبر AJAX
// =========================================================
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_new_app'])) {
    ...
    try {
        // التحقق العميق ومطابقة الصك مع وزارة العدل (DSS validation)
        ...
        if ($mojRecord['owner_national_id'] !== $nationalId) {
            // إذا لم تتطابق الهوية، يتم تحويل الطلب تلقائياً للمراجعة اليدوية من قبل المدقق
            $appStatus = 'Pending_Review';
            $notifMsg = "تم تقديم الطلب رقم صك {$deedNumber} بنجاح، وتحويله للمدقق اليدوي لمطابقة اختلاف بيانات المالك.";
        } else {
            // إذا تطابقت 100%، يمر الطلب للمسار السريع الفوري (الفحص الميداني)
        ...
        // رفع الملف وتشفير اسمه لحمايته أمنياً
        ...
        if (move_uploaded_file($fileTmpPath, $targetFilePath)) {
            $servicesToInsert = [];
            if ($srvId == 3) {
                $servicesToInsert = [1, 2]; // مياه وصرف منفصلين
            } else {
            ...
            foreach ($servicesToInsert as $singleSrvId) {
                // التحقق من عدم وجود طلب مكرر نشط لنفس الخدمة على هذا الصك
                ...
                $srvNameClean = ($singleSrvId == 1) ? 'شبكة مياه' : 'صرف صحي';
                $appsDetails[] = "طلب {$srvNameClean} رقم (#" . str_pad($newAppId, 5, '0', STR_PAD_LEFT) . ")";

                // إذا مر الطلب للفحص الميداني، نقوم بإسناده آلياً لفني فحص في نفس المدينة يمتلك أقل مهام
                if ($appStatus == 'Pending_Inspection') {
                    $bestTechStmt = $pdo->prepare("
                        ...
                        ORDER BY ce.active_tasks_count ASC LIMIT 1
                    ");
                    $bestTechStmt->execute([$cityId]);
                    ...
                    if ($bestTechId) {
                        ...
                        $stmtTechName->execute([$bestTechId]);
                        $techName = $stmtTechName->fetchColumn() ?? 'فني قطرة';

                        $personalNotifMsg = "تهانينا! تم التحقق من صك الملكية آلياً بنجاح عبر محرك (DSS). تم توجيه طلبك المخصص لـ (" . cleanServiceName($srvNameClean) . ") رقم #" . str_pad($newAppId, 5, '0', STR_PAD_LEFT) . " للفحص الميداني وإسناد المهمة للموظف المتميز: " . $techName;
                        $pdo->prepare("INSERT INTO notification (message_content, cust_id) VALUES (?, ?)")->execute([$personalNotifMsg, $custId]);
                    } else {
            ...
            $successMsg = ($srvId == 3) 
                ? "تم تقديم طلب جديد لعقارك بنجاح وتقسيمه إلى طلبين تشغيليين لتسريع الإجراءات والربط الميداني: " . implode(" و ", $appsDetails)
                : "تم تقديم طلبك بنجاح وجاري معالجته آلياً عبر نظام قطرة الذكي.";

            echo json_encode(['status' => 'success', 'message' => $successMsg]);
            exit;
        } else {
            echo json_encode(['status' => 'error', 'message' => 'فشل في رفع ملف الصك، يرجى التحقق من الصلاحيات وحجم الملف.']);
            exit;
        }

    } catch (PDOException $e) {
        echo json_encode(['status' => 'error', 'message' => 'حدث خطأ غير متوقع أثناء معالجة وحفظ الطلب.']);
        exit;
    }
}

// =========================================================
// 2. معالجة سداد الفاتورة (Mock Payment & Provisioning Engine)
// =========================================================
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['pay_invoice'])) {
    ...
    try {
        ...
        // جلب تفاصيل الفاتورة والطلب للتأكد من جاهزيتها للسداد
        ...
        if ($appData) {
            // أ. تحديث حالة الفاتورة لتصبح مدفوعة
            $pdo->prepare("UPDATE invoice SET payment_status = 'Paid' WHERE inv_id = ?")->execute([$appData['inv_id']]);

            // ب. تحديث حالة الطلب العام ليصبح جاري التنفيذ والتركيب
            $pdo->prepare("UPDATE application SET app_status = 'In_Progress' WHERE app_id = ?")->execute([$appId]);

            // جـ. تفعيل الحساب الموحد للعقار (Unified Account) لربط كافة الخدمات والعدادات به مستقبلاً
            ...
            // د. ربط وتفعيل نوع الخدمة المطلوبة بالحساب الموحد الجديد
            $pdo->prepare("INSERT INTO activated_service (acc_id, srv_id) VALUES (?, ?)")->execute([$accId, $appData['srv_id']]);

            // هـ. التوجيه الذكي التلقائي لفني التركيب لتركيب العداد وربط البيانات
            // تبدأ خوارزمية التوزيع بالبحث في مدينة العميل، ثم باقي مدن المنطقة، واختيار الأقل ضغطاً
            ...
            if ($techId) {
                // إنشاء مهمة التركيب وإسنادها للفني المختار
                $pdo->prepare("INSERT INTO installation_task (app_id, emp_id) VALUES (?, ?)")->execute([$appId, $techId]);
                // زيادة عداد المهام النشطة للفني
                $pdo->prepare("UPDATE company_employee SET active_tasks_count = active_tasks_count + 1 WHERE emp_id = ?")->execute([$techId]);
            }

            // جلب اسم فني التركيبات المسند إليه الطلب
            $techName = 'فني التوصيل المعتمد';
            if ($techId) {
                $stmtTechName = $pdo->prepare("SELECT emp_name FROM company_employee WHERE emp_id = ?");
                $stmtTechName->execute([$techId]);
                $techName = $stmtTechName->fetchColumn() ?? 'فني قطرة';
            }

            // و. حقن الإشعار الآلي بنجاح العملية
            $srvNameClean = ($appData['srv_id'] == 1) ? 'شبكة مياه' : 'صرف صحي';
            $notifMsg = "تم تأكيد سداد فاتورة الطلب # " . str_pad($appId, 5, '0', STR_PAD_LEFT) . " بقيمة " . number_format($appData['amount'], 2) . " ريال بنجاح. تم إصدار حسابك الموحد (ACC-" . str_pad($accId, 5, '0', STR_PAD_LEFT) . ") وإسناد مهمة تركيب عداد (" . cleanServiceName($srvNameClean) . ") وتوصيل الخدمة إلى فني التركيبات المتميز: " . $techName;
            $pdo->prepare("INSERT INTO notification (message_content, cust_id) VALUES (?, ?)")->execute([$notifMsg, $custId]);

            // ز. كتابة السجل في التاريخ العام للطلبات
            $pdo->prepare("INSERT INTO application_history (app_id, status, change_date) VALUES (?, 'In_Progress', NOW())")->execute([$appId]);

            $pdo->commit();
            echo json_encode(['status' => 'success', 'message' => 'تم تأكيد السداد وإصدار الحساب الموحد وتفعيل خطة التركيبات بنجاح!']);
            exit;
        } else {
            $pdo->rollBack();
            echo json_encode(['status' => 'error', 'message' => 'عفواً، لم يتم العثور على الفاتورة النشطة، أو ربما تم دفعها مسبقاً.']);
            exit;
        }
    } catch (Exception $e) {
        $pdo->rollBack();
        echo json_encode(['status' => 'error', 'message' => 'فشلت عملية الفوترة والسداد الموحدة: ' . $e->getMessage()]);
        exit;
    }
}

// =========================================================
// 3. جلب كافة البيانات المطلوبة لواجهة المستفيد
// =========================================================
$citiesWithRegions = [];
$services = [];
$myApplications = [];
...
try {
    // جلب المدن والمناطق لنموذج التقديم والخريطة
    $citiesWithRegions = $pdo->query("SELECT c.cty_id, c.cty_name, r.reg_name FROM city c JOIN region r ON c.reg_id = r.reg_id WHERE c.cty_name NOT IN ('الربيعية', 'الشماسية', 'الربيعيه', 'الشماسيه', 'رفحاء') ORDER BY r.reg_id, c.cty_name")->fetchAll(PDO::FETCH_ASSOC);
    // جلب أنواع الخدمات المتاحة للتقديم
    $services = $pdo->query("SELECT srv_id, srv_name FROM service_type")->fetchAll(PDO::FETCH_ASSOC);

    // جلب طلبات المستفيد الحالية مع فواتيرها إن وجدت
    $appStmt = $pdo->prepare("
        SELECT a.app_id, s.srv_name, a.deed_no, a.app_status, a.created_at, c.cty_name,
               i.amount, i.payment_status,
               moj.owner_name, moj.land_area
        FROM application a
        JOIN city c ON a.cty_id = c.cty_id
        JOIN service_type s ON a.srv_id = s.srv_id
        JOIN moj_record moj ON a.deed_no = moj.deed_no
    ...
    // حساب إحصائيات المستفيد السريعة
    ...
    // جلب الحسابات الموحدة المميزة أولاً لتفادي تكرار الواجهة
    $propStmt = $pdo->prepare("
        SELECT DISTINCT ua.acc_id, ua.deed_no, ua.creation_date, moj.land_area, moj.owner_name
    ...
    $rawProperties = $propStmt->fetchAll(PDO::FETCH_ASSOC);

    $myProperties = [];
    foreach ($rawProperties as $pRow) {
        // جلب الخدمات المفعّلة المميزة لهذا الحساب الموحد
        $srvStmt = $pdo->prepare("
            SELECT DISTINCT st.srv_name 
            FROM activated_service acs
            JOIN service_type st ON acs.srv_id = st.srv_id
            WHERE acs.acc_id = ?
        ");
        $srvStmt->execute([$pRow['acc_id']]);
        $pRow['services'] = $srvStmt->fetchAll(PDO::FETCH_ASSOC);

        // جلب العدادات المرتبطة بهذا الحساب
        $mtrStmt = $pdo->prepare("
            SELECT DISTINCT m.mtr_serial, m.mtr_type, st.srv_name as service_name
            FROM meter m
            LEFT JOIN installation_task it ON m.task_id = it.task_id
            LEFT JOIN application a ON it.app_id = a.app_id
            LEFT JOIN service_type st ON a.srv_id = st.srv_id
            WHERE m.acc_id = ?
        ");
        $mtrStmt->execute([$pRow['acc_id']]);
        $pRow['meters'] = $mtrStmt->fetchAll(PDO::FETCH_ASSOC);

        $myProperties[] = $pRow;
    }

    // جلب إشعارات المستفيد
    ...
    // معالجة صامتة لتجنب إتلاف الاتصال
}

// دالة جلب شارات حالات الطلب الاحترافية بتصميم متطابق
function getStatusBadge($status) {
    $badges = [
        'Pending_Review' => '<span class="status-badge badge-warning"><i class="fa-solid fa-file-signature"></i> قيد المراجعة الأمنية</span>',
        'Pending_Inspection' => '<span class="status-badge badge-info"><i class="fa-solid fa-helmet-safety"></i> جاري جدولة الفحص الميداني</span>',
        'Pending_Billing' => '<span class="status-badge badge-dark"><i class="fa-solid fa-file-invoice-dollar"></i> بانتظار سداد الفاتورة</span>',
        'In_Progress' => '<span class="status-badge badge-primary"><i class="fa-solid fa-person-digging"></i> جاري تركيب العداد الذكي</span>',
        'Completed' => '<span class="status-badge badge-success"><i class="fa-solid fa-circle-check"></i> مكتمل ومفعّل بالكامل</span>',
        'Rejected' => '<span class="status-badge badge-danger"><i class="fa-solid fa-circle-xmark"></i> طلب مرفوض</span>'
    ];
...
<head>
    ...
    <style>
        ...
        /* تأثيرات الخلفية والفقاعات المائية الملتزم بها */
...
<body>
    ...
    <nav class="navbar navbar-luxury fade-in-up">
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <a href="login.php?logout=1" class="btn btn-light text-danger rounded-circle shadow-sm border btn-logout" title="تسجيل الخروج"><i class="fa-solid fa-power-off"></i></a>
            </div>
        </div>
    </nav>

    <div class="container pb-5 mt-4">
        <!-- ترحيب المشترك والإحصائيات السريعة -->
        ...
        <!-- أزرار تبويب الواجهة الاحترافية -->
        <div class="custom-tabs fade-in-up delay-2">
            ...
            <button class="tab-btn" onclick="switchTab('tab-properties', this)"><i class="fa-solid fa-hotel me-1"></i> أصولي العقارية والعدادات</button>
            <button class="tab-btn" onclick="switchTab('tab-notifs', this)"><i class="fa-solid fa-bell me-1"></i> مركز الإشعارات</button>
        </div>

        <!-- حاوية البطاقة الكبرى -->
        <div class="premium-card fade-in-up delay-3">
                        ...
                        <div class="col-md-6">
                            <div class="mb-4">
                                ...
                                <select name="srv_id" class="form-select" required>
                                    ...
                                    <?php foreach ($services as $srv): ?><option value="<?= $srv['srv_id']; ?>"><?= htmlspecialchars(cleanServiceName($srv['srv_name'])); ?></option><?php endforeach; ?>
                                </select>
                            </div>
                            <div class="mb-4">
                                <label class="form-label fw-bold">مدينة العقار <span class="text-danger">*</span></label>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="map-container" id="mapContainer">
                                <div class="map-overlay" id="mapLock">
                                    ...
                                    <span class="fw-bold">الرجاء اختيار مدينة العقار أولاً لتفعيل الخريطة</span>
                                </div>
                            ...
                            <tbody>
                                <?php foreach ($myApplications as $app): ?>
                                    <tr>
                                        ...
                                        <td><div class="fw-bold"><?= htmlspecialchars($app['srv_name']); ?></div><div class="small text-muted"><i class="fa-solid fa-location-dot text-danger"></i> <?= htmlspecialchars($app['cty_name']); ?></div></td>
                                        ...
                                        <td>
                                            <?php if ($app['payment_status'] == 'Unpaid'): ?>
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    <!-- زر دفع تجريبي مدمج يعرض بوابة دفع فورية بـ SweetAlert -->
                                                    <button class="btn btn-sm btn-success fw-bold px-3 rounded-pill" onclick="simulatePayment(<?= $app['app_id']; ?>, <?= $app['amount']; ?>, '<?= htmlspecialchars($app['deed_no']); ?>', '<?= htmlspecialchars($app['owner_name']); ?>', <?= floatval($app['land_area']); ?>, '<?= htmlspecialchars(cleanServiceName($app['srv_name'])); ?>')"><i class="fa-solid fa-credit-card"></i> سداد سريع</button>
                                                </div>
            ...
            <!-- التبويب 3: أصولي العقارية والعدادات المفعّلة -->
            <div id="tab-properties" class="tab-panel">
                <div class="card-header-title"><i class="fa-solid fa-building-circle-check"></i> سجل الأصول والعدادات الذكية المفعّلة</div>
                            ...
                            <div class="col-md-6">
                                <div class="property-box bg-white">
                                    ...
                                    <div class="border-top pt-3">
                                        ...
                                        <p class="mb-2"><strong>الخدمات المربوطة:</strong> 
                                            <?php foreach ($prop['services'] as $srvItem): ?>
                                                <span class="badge bg-light text-primary border me-1"><?= htmlspecialchars(cleanServiceName($srvItem['srv_name'])); ?></span>
                                            <?php endforeach; ?>
                                        </p>
                                    </div>
                                    
                                    <div class="mt-3">
                                        <div class="fw-bold text-secondary mb-2"><i class="fa-solid fa-microchip text-success me-1"></i> بيانات العدادات الميدانية الموثقة:</div>
                                        <?php if(!empty($prop['meters'])): ?>
                                            <?php foreach ($prop['meters'] as $meter): ?>
                                                <div class="bg-light rounded p-3 mb-2 border">
                                                    <div class="d-flex justify-content-between mb-1">
                                                        <span class="small text-muted">الخدمة المرتبطة</span>
                                                        <span class="badge bg-primary text-white"><?= htmlspecialchars(cleanServiceName($meter['service_name'] ?? 'شبكة مياه')); ?></span>
                                                    </div>
                                                    <div class="row text-center">
                                                        <div class="col-6 border-end">
                                                            <div class="small text-muted">الرقم التسلسلي</div>
                                                            <div class="fw-bold font-monospace text-dark"><?= htmlspecialchars($meter['mtr_serial']); ?></div>
                                                        </div>
                                                        <div class="col-6">
                                                            <div class="small text-muted">نوع وموديل العداد</div>
                                                            <div class="fw-bold text-dark"><?= $meter['mtr_type'] == 'Smart' ? 'ذكي' : 'ميكانيكي' ?></div>
                                                        </div>
                                                    </div>
                                                </div>
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <div class="alert alert-warning text-center fw-bold mt-2 mb-0"><i class="fa-solid fa-spinner fa-spin"></i> جاري جدولة الفني لتركيب العداد وربط البيانات</div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        // تفعيل الفقاعات المائية المتحركة الممتعة بصرياً
        ...
        // التبديل السلس بين التبويبات الفاخرة
        ...
        // برمجة وتحديث خريطة موقع العقار
        ...
        map.on('click', function(e) {
            ...
            document.getElementById('gpsStatus').innerHTML = `<span class="text-success fw-bold"><i class="fa-solid fa-circle-check"></i> تم التقاط الإحداثيات بنجاح: ${lat.toFixed(4)}, ${lng.toFixed(4)}</span>`;
        });

        // رفع الطلب الجديد عبر AJAX ومطابقة الصكوك فورياً
        document.getElementById('applicationForm').addEventListener('submit', function(e) {
            e.preventDefault();
            if (document.getElementById('latitude').value === "") {
                Swal.fire({ icon: 'warning', title: 'تحديد الموقع مهم!', text: 'يرجى النقر على الخريطة لتحديد مكان العقار الجغرافي.', confirmButtonColor: '#092e54' });
                return;
            }
            ...
            Swal.fire({
                title: 'جاري مراجعة الطلب ومطابقة الصك...',
                text: 'يقوم محرك (DSS) بمطابقة البيانات مع سجلات وزارة العدل وتشفير المرفقات آلياً.',
        ...
        // محاكاة سداد الفاتورة وتأكيدها آلياً
        function simulatePayment(appId, amount, deedNo, ownerName, landArea, srvName) {
            let baseAmount = amount / 1.15;
            let vatAmount = amount - baseAmount;
            
            Swal.fire({
                title: 'بوابة الدفع الإلكتروني الموحدة (قطرة)',
                html: `
                    <div style="text-align: right; font-family: 'Cairo', sans-serif; font-size: 0.9rem; color: #1e293b;">
                        <div class="mb-3 p-3 text-center" style="background: linear-gradient(135deg, #092e54, #4492d4); color: white; border-radius: 12px; box-shadow: 0 4px 10px rgba(9,46,84,0.15);">
                            <i class="fa-solid fa-droplet fs-2 mb-2 d-block text-info"></i>
                            <h5 class="fw-black m-0">تفاصيل وسداد الفاتورة الموحدة</h5>
                            <span class="small opacity-75">طلب رقم: #${appId}</span>
                        </div>
                        
                        <div class="p-3 bg-light rounded-3 mb-3 border">
                            <h6 class="fw-black text-dark border-bottom pb-2 mb-2"><i class="fa-solid fa-file-invoice text-primary me-1"></i> معلومات الصك والعقار</h6>
                            <p class="mb-1"><strong>نوع الخدمة:</strong> <span class="text-primary fw-bold">${srvName}</span></p>
                            <p class="mb-1"><strong>رقم صك الملكية:</strong> <span class="font-monospace text-muted">${deedNo}</span></p>
                            <p class="mb-1"><strong>صاحب العقار الموثق:</strong> <span class="fw-bold">${ownerName}</span></p>
                            <p class="mb-0"><strong>المساحة الإجمالية:</strong> <span class="fw-bold text-dark">${landArea} م²</span></p>
                        </div>

                        <div class="p-3 bg-light rounded-3 mb-3 border">
                            <h6 class="fw-black text-dark border-bottom pb-2 mb-2"><i class="fa-solid fa-receipt text-success me-1"></i> الملخص المالي</h6>
                            <div class="d-flex justify-content-between mb-1">
                                <span class="text-muted">رسوم التأسيس والتوصيل (قبل الضريبة):</span>
                                <span class="fw-bold">${baseAmount.toFixed(2)} ر.س</span>
                            </div>
                            <div class="d-flex justify-content-between mb-1">
                                <span class="text-muted">ضريبة القيمة المضافة (15%):</span>
                                <span class="fw-bold">${vatAmount.toFixed(2)} ر.س</span>
                            </div>
                            <div class="d-flex justify-content-between border-top pt-2 mt-2" style="font-size: 1.1rem;">
                                <span class="fw-black text-dark">المبلغ الإجمالي مستحق السداد:</span>
                                <span class="fw-black text-success">${amount.toLocaleString()} ر.س</span>
                            </div>
                        </div>

                        <div class="p-3 rounded-3 mb-3 border" style="background: #f0fdf4; border-color: #bbf7d0;">
                            <h6 class="fw-black text-success border-bottom pb-2 mb-2"><i class="fa-solid fa-credit-card me-1"></i> طريقة الدفع المفضلة</h6>
                            <div class="d-flex justify-content-around my-2">
                                <label class="fw-bold" style="cursor: pointer;">
                                    <input type="radio" name="pay_method" value="mada" checked>
                                    <b>مدى (Mada)</b>
                                </label>
                                <label class="fw-bold" style="cursor: pointer;">
                                    <input type="radio" name="pay_method" value="visa">
                                    <b>فيزا / ماستركارد</b>
                                </label>
                            </div>
                            <div class="mt-3">
                                <input type="text" class="form-control form-control-sm mb-2" placeholder="اسم صاحب البطاقة" style="border-radius: 8px;" required>
                                <input type="text" class="form-control form-control-sm mb-2" placeholder="رقم البطاقة (16 رقم)" style="border-radius: 8px;" maxlength="19" required>
                                <div class="row g-2">
                                    <div class="col-6">
                                        <input type="text" class="form-control form-control-sm" placeholder="تاريخ الانتهاء MM/YY" style="border-radius: 8px;" maxlength="5" required>
                                    </div>
                                    <div class="col-6">
                                        <input type="password" class="form-control form-control-sm" placeholder="رمز الأمان CVV" style="border-radius: 8px;" maxlength="3" required>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                confirmButtonText: 'تأكيد السداد الآمن',
