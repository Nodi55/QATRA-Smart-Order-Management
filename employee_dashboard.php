...
$adminTasksCount = 0;
try {
    $adminTasksCount = $pdo->query("
        SELECT COUNT(*) FROM application a 
        LEFT JOIN field_inspection fi ON a.app_id = fi.app_id 
        WHERE a.app_status = 'Pending_Inspection' AND fi.insp_id IS NULL
    ")->fetchColumn();
} catch (Exception $e) { $adminTasksCount = 0; }

// 2. صلاحية مدقق الطلبات: عدد طلبات المطابقة اليدوية بانتظار التحقق (Pending Review)
$auditorTasksCount = 0;
try {
    $auditorTasksCount = $pdo->query("
        SELECT COUNT(*) FROM application WHERE app_status = 'Pending_Review'
    ")->fetchColumn();
} catch (Exception $e) { $auditorTasksCount = 0; }

// 3. صلاحية فني فحص: عدد مهام المعاينة الفحص الميداني المسندة ولم تنجز بعد
$inspectTasksCount = 0;
try {
    $inspectStmt = $pdo->prepare("
        SELECT COUNT(*) FROM field_inspection 
        WHERE emp_id = ? AND inspection_result IS NULL
    ");
    $inspectStmt->execute([$empId]);
    $inspectTasksCount = $inspectStmt->fetchColumn();
} catch (Exception $e) { $inspectTasksCount = 0; }

// 4. صلاحية فني تركيب: عدد مهام التركيب الميداني وربط العدادات المسندة المعلقة
$installTasksCount = 0;
try {
    $installStmt = $pdo->prepare("
        SELECT COUNT(*) FROM installation_task 
        WHERE emp_id = ? AND initial_reading IS NULL
    ");
    $installStmt->execute([$empId]);
    $installTasksCount = $installStmt->fetchColumn();
} catch (Exception $e) { $installTasksCount = 0; }

// =========================================================
// معالجة قراءة وقبول الإشعارات والتحذيرات الإدارية المباشرة
// =========================================================
...
// جلب إشعارات الموظف الحالي غير المقروءة لتظهر في مركز الإشعارات الموحد
$empNotifs = [];
try {
...
<div class="col-lg-12">
<div class="card border-0 shadow rounded-4" style="background: linear-gradient(135deg, #fffbeb 0%, #fff7ed 100%); border-right: 6px solid #f97316 !important;">
<div class="card-body p-4">
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
<h5 class="fw-black text-warning m-0"><i class="fa-solid fa-bell fa-shake me-2"></i> مركز التنبيهات والإنذارات الإدارية للموظف</h5>
...
$badgeLabel = $isWarning ? 'إنذار إداري عاجل' : 'توزيع جغرافي ذكي';
?>
<div class="p-3 bg-white rounded-3 border d-flex justify-content-between align-items-center mb-2 shadow-sm text-end" style="direction: rtl;">
<div class="d-flex align-items-center gap-3">
...
<!-- ========================================================= -->
<!-- شبكة مساحات العمل والعدادات الذكية المعلقة لكل صلاحية -->
<!-- ========================================================= -->
<div class="roles-section fade-in-up delay-3">
<div class="roles-grid">
<?php if(in_array('Admin', $roles)): ?>
<a href="?active_role=Admin" class="role-card">
    <?php if($adminTasksCount > 0): ?>
        <span class="badge bg-danger rounded-pill px-3 py-1 position-absolute" style="top: 15px; left: 15px; font-weight: 800; font-size: 0.8rem; box-shadow: 0 4px 10px rgba(220,53,69,0.3);">
            <?= $adminTasksCount; ?> مهام للتوزيع
        </span>
    <?php else: ?>
        <span class="badge bg-success rounded-pill px-3 py-1 position-absolute" style="top: 15px; left: 15px; font-weight: 800; font-size: 0.8rem;">
            مكتمل ✓
        </span>
    <?php endif; ?>
...
<a href="?active_role=Auditor" class="role-card">
    <?php if($auditorTasksCount > 0): ?>
        <span class="badge bg-danger rounded-pill px-3 py-1 position-absolute" style="top: 15px; left: 15px; font-weight: 800; font-size: 0.8rem; box-shadow: 0 4px 10px rgba(220,53,69,0.3);">
            <?= $auditorTasksCount; ?> بانتظار التدقيق
        </span>
    <?php else: ?>
        <span class="badge bg-success rounded-pill px-3 py-1 position-absolute" style="top: 15px; left: 15px; font-weight: 800; font-size: 0.8rem;">
            مكتمل ✓
...
<a href="?active_role=<?= urlencode('Inspection Technician') ?>" class="role-card">
    <?php if($inspectTasksCount > 0): ?>
        <span class="badge bg-danger rounded-pill px-3 py-1 position-absolute" style="top: 15px; left: 15px; font-weight: 800; font-size: 0.8rem; box-shadow: 0 4px 10px rgba(220,53,69,0.3);">
            <?= $inspectTasksCount; ?> مهام فحص نشطة
        </span>
    <?php else: ?>
        <span class="badge bg-success rounded-pill px-3 py-1 position-absolute" style="top: 15px; left: 15px; font-weight: 800; font-size: 0.8rem;">
            مكتمل ✓
...
<a href="?active_role=<?= urlencode('Installation Technician') ?>" class="role-card">
    <?php if($installTasksCount > 0): ?>
        <span class="badge bg-danger rounded-pill px-3 py-1 position-absolute" style="top: 15px; left: 15px; font-weight: 800; font-size: 0.8rem; box-shadow: 0 4px 10px rgba(220,53,69,0.3);">
            <?= $installTasksCount; ?> مهام تركيب نشطة
        </span>
    <?php else: ?>
        <span class="badge bg-success rounded-pill px-3 py-1 position-absolute" style="top: 15px; left: 15px; font-weight: 800; font-size: 0.8rem;">
            مكتمل ✓
